...
from ame_manager_app.equipment.forms import (
    BorrowMultipleEquipmentsForm,
...
def add_register_new_equipment():
    ...
    form.responsible_user.choices = [
        (user.id, user.name)
        for user in Users.query.filter(Users.role_code == ADMIN).all()
    ]
    if current_user.role_code == ADMIN:
        form.responsible_user.default = current_user.id
    form.storage_location_id.choices = [
        (storage.id, str(storage.name) + " in " + str(storage.room_id))
        for storage in StorageModel.query.all()
    ]
    ...
    if form.validate_on_submit():
        ...
        _storage_location = StorageModel.query.filter_by(
            id=form.storage_location_id.data
        ).first()
...
def search():
    ...
    _form_search.equipment_id.choices = [
        (equipment.id, equipment.name) for equipment in equipments
    ]

    _form_filter = FilterEquipmentForm()
    _form_filter.room.choices = [(room.id, room.name) for room in rooms]
    _form_filter.storage_location.choices = [
        (storage.id, storage.name) for storage in storages
    ]
    _form_filter.usage_location.choices = [
        (storage.id, storage.name) for storage in storages
    ]
    ...
    if _form_filter.validate_on_submit():
        ...
        if mode == "Room ":
            ...
            equipments = EquipmentModel.query.filter(
                EquipmentModel.storage_location_id in _storage_locations
            ).all()
            _form_search.equipment_id.choices = [
                (equipment.id, equipment.name) for equipment in equipments
            ]
        if mode == "Storage location ":
            _storage_location = StorageModel.query.filter_by(
                id=_form_filter.storage_location.data
            ).first()
            equipments = EquipmentModel.query.filter(
                EquipmentModel.storage_location_id in _storage_locations
            ).all()
        ...
        if mode == "Usage location ":
            _usage_location = StorageModel.query.filter_by(
                id=_form_filter.usage_location.data
            ).first()
            equipments = EquipmentModel.query.filter(
                EquipmentModel.get_current_active_usage().usage_location_id
                in _usage_location
            ).all()
        ...
        if _form_search.validate_on_submit():
            _equipment = EquipmentModel.query.filter_by(
                id=_form_search.equipment_id.data
            ).first()
            _resp_user = Users.query.filter(
                Users.id == _equipment.responsible_user_id
            ).first()
...
def my_page():
    _usages = UsageModel.query.filter_by(user_id=current_user.id).all()
    _responsible_equipments = EquipmentModel.query.filter_by(
        responsible_user_id=current_user.id
    ).all()
    _responsible_storages = StorageModel.query.filter_by(
        responsible_user_id=current_user.id
    ).all()
    _responsible_rooms = RoomModel.query.filter_by(
        responsible_user_id=current_user.id
    ).all()

    return render_template(
        "equipment/my_page.html",
        user=current_user,
...
def view_room(id):
    ...
    for _storage in _storages:
        _all_equipment_from_storages.append(
            EquipmentModel.query.filter(
                EquipmentModel.storage_location_id == _storage.id
            ).all()
        )
    return render_template(
        "equipment/room_page.html",
        room=_room,
        resp_user=_resp_user,
        storages=_storages,
...
def borrow_equipment(id):
    ...
    _form.usage_location_id.choices = [
        (storage.id, storage.name) for storage in storages
    ]
    _form.alt1_usage_planned_end_date.data = datetime.date.today() + datetime.timedelta(
        days=1
    )
    ...
    _form.borrowing_equipment.choices = [
        (equipment.id, equipment.name) for equipment in is_not_in_use_equipments
    ]
    _form.borrowing_equipment.default = (
        _equipment.id
    )  # (_equipment.id, _equipment.name)
    ...
    if _equipment.is_in_use():
        flash(
            f"Equipment {_equipment.name} with id {id} is currently in use by {_equipment.get_current_active_usage().user}!",
            "danger",
        )
        return redirect(
            "/equipment/my_page"
    ...
    if _equipment.is_usable == False:
        ...
        flash(
            f"Equipment {_equipment.name} with id {id} is currently not usable. Please contact {_resp_user.name}!",
            "danger",
        )
        return redirect(
            "/equipment/my_page"
    ...
    if _form.validate_on_submit():
        _equipment = EquipmentModel.query.filter_by(
            id=_form.borrowing_equipment.data
        ).first()
        _user = Users.query.filter_by(id=_form.user.data).first()
        _usage_location = StorageModel.query.filter_by(
            id=_form.usage_location_id.data
        ).first()
    ...
    return render_template(
        "equipment/borrow_equipment.html", form=_form, equipment=_equipment
    )
...
def borrow_multiple_equipments():
    ...
    _form.name.data = "experimentname"
    _form.user.choices = [(user.id, user.name) for user in users]
    _form.borrowing_equipments.choices = [
        (equipment.id, equipment.name) for equipment in is_not_in_use_equipments
    ]
    ...
    if _form.validate_on_submit():
        ...
        for _equipment_id in _borrowing_equipments:
            ...
            if _equipment_id is None:
                ...
                _equipment = EquipmentModel.query.filter_by(
                    id=int(_equipment_id)
                ).first()
                bool_stopper = False
                if _equipment is None:
                    flash(
                        f"Equipment {_equipment.name} with id {_equipment.id} does not exist!",
                        "danger",
                    )
                    bool_stopper = True
                ...
                if _equipment.is_in_use():
                    flash(
                        f"Equipment {_equipment.name} with id {_equipment.id} is currently in use by {_equipment.get_current_active_usage().user}!",
                        "danger",
                    )
                    bool_stopper = True
                ...
                if _equipment.is_usable == False:
                    ...
                    flash(
                        f"Equipment {_equipment.name} with id {_equipment.id} is currently not usable. Please contact {_resp_user.name}!",
                        "danger",
                    )
                    bool_stopper = True
                ...
                if bool_stopper == False:
                    ...
                    flash(
                        f"Borrowing Equipment {_equipment.name} with id {_equipment.id} was successfull.",
                        "success",
                    )
...
def return_equipment(id):
    ...
    if _equipment.is_in_use() == False:
        flash(
            f"Equipment {_equipment.name} with id {id} cannot be returned, since it has not been borrowed.",
            "danger",
        )
        return redirect(url_for("equipment.my_page"))

    if (
        not _equipment.is_in_use_by(current_user)
        and not current_user.role_code == ADMIN
    ):
        flash(
            f"Equipment {_equipment.name} with id {id} has not been borrowed by you.",
            "danger",
        )
        return redirect(url_for("equipment.my_page"))
...
def add_briefing(id):
    ...
    if not current_user.role_code == ADMIN:
        flash(
            f"Sorry, Admin only can add briefings this equipment {_equipment.name} with id {id}. Please contact {_resp_user.name}!",
            "danger",
        )
        return redirect(url_for("equipment.my_page"))
...
def add_calibration(id):
    ...
    return render_template(
        "equipment/add_calibration.html",
        form=_form,
        equipment=_equipment,
    )
