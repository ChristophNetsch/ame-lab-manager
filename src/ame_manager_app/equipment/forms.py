...
class RegisterEquipmentForm(FlaskForm):
    ...
    is_calibration_nessessary = BooleanField(
        " Calibration for use is necessary", validators=[Optional()]
    )
    is_briefing_nessessary = BooleanField(
        " Briefing for user is necessary", validators=[Optional()]
    )
...
class BorrowEquipmentForm(FlaskForm):
    ...
    usage_start_datetime = DateTimeField(
        "Usage start date",
